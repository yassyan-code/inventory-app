...
- **RLSは`team_id`単位で分離済み**（第16回）。さらに第17回で`role`による書き込み制限を追加（`owner`のみ商品マスタを操作可）。詳細は[認証メモ](auth.md)と`supabase/006_add_multitenancy.sql` / `007_role_write_policies.sql`
...
- 同じチームに2人目以降を招待するフローが未実装（サインアップ＝1人1チームのまま。詳細は[認証メモ](auth.md)）
- 課金は第18回で Stripe を導入（[課金メモ](billing.md)）。第19回で継続課金の運用（請求失敗・解約）をWebhookで詰める予定
- 在庫のしきい値通知なし
